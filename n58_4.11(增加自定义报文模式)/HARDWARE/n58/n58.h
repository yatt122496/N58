    ...
    struct 
		{
		  ...
		  __IO u8 bFinish_state				:1;			// 2		命令完成状态标志，0为发送失败
			...
			__IO u8 bResponse_Flag			:3;			// 5-7	应答类别，0为不需要应答，
																					//							 	1为普通应答
																					//								大于1为特殊应答
	  }bInfBit;
  }; 
...
void N58_Load_Cmd(u8 bLen,u8 *bN58_Cmd_List,u8 *bN58_Ack_List);		//加载待发送命令函数

void N58_printf(char* fmt,...);							//printf函数
void N58_Usart_Send(u8 *bData,u16 bLen);			//N58发送字节函数

void N58_Init(void);												//N58模块通讯初始化
void N58_Ppp_Link(void);										//N58 PPP 连接函数
void N58_Http_Set(u8 *bUrl,u8 *bPort);					//设置 N58 h
...
void N58_Http_Open_Mode(u8 *bCode,u8 bLen);			//N58 http openmode 自定义报文模式
void N58_Http_Close(void);											//N58 http Close 关闭
