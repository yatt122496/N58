...
void N58_Send_AT_Cmd(u8 *bCmd,u8 *bAck)
{
	...
	if(bCmd==bN58_Data_Post)
		N58_Usart_Send(&bCmd[1],bCmd[0]);
	else
	{
		N58_printf("%s\r\n",bCmd);
		printf("%s\r\n",bCmd);
...
void N58_Load_Cmd(u8 bLen,u8 *bN58_Cmd_List,u8 *bN58_Ack_List)
{
	u8 i;
...
void N58_Init(void)
{
	...
	N58_Load_Cmd(14,bN58_Cmd_List,bN58_Ack_List);
}
...
void N58_Ppp_Link(void)
{
	...
	N58_Load_Cmd(3,bN58_Cmd_List,bN58_Ack_List);
}
...
{
	u8 bN58_Cmd_List[2+1]={Command16,Command17,CommandNull};
...
//	注意接收的内容不要超过接收缓冲区大小
void N58_Http_Get(u8 *bOffset,u8 *bSize)
{
	u8 bN58_Cmd_List[2+1]={Command18,Command19,CommandNull};
	u8 bN58_Ack_List[2+1]={Command1,Command1,CommandNull};
	if(bOffset&&bSize)
	{
		bN58_Cmd_List[1]=Command20;
...
//HTTP HEAD 请求方法
void N58_Http_Head(void)
{
	u8 bN58_Cmd_List[2+1]={Command18,Command21,CommandNull};
	u8 bN58_Ack_List[2+1]={Command1,Command1,CommandNull};
	N58_Load_Cmd(3,bN58_Cmd_List,bN58_Ack_List);
}
...
//bCode				：发送的数据
//bLen				：发送的数据长度
void N58_Http_Post(u8 bType,u8 *bCode,u8 bLen)
{
	u8 i;
	u8 bN58_Cmd_List[2+1]={Command18,Command25,CommandNull};
	u8 bN58_Ack_List[2+1]={Command1,Command8,CommandNull};
	if(bType>4)
		bType=1;
	sprintf((char *)bN58_AT_Cmd_List[Command25],"AT+HTTPACTION=2,%d,%d",bLen,bType);
	bN58_Data_Post[0]=bLen;
	for(i=0;i<bLen;i++)
		bN58_Data_Post[i+1]=bCode[i];
	N58_Load_Cmd(3,bN58_Cmd_List,bN58_Ack_List);
	bN58_Data_List[0]=bN58_Data_Post;
	bN58_Data_List[1]=0;
}

//HTTP OpenMode 自定义报文 请求方法
//bCode				：发送的数据
//bLen				：发送的数据长度
void N58_Http_Open_Mode(u8 *bCode,u8 bLen)
{
	u8 i;
	u8 bN58_Cmd_List[2+1]={Command18,Command25,CommandNull};
	u8 bN58_Ack_List[2+1]={Command1,Command8,CommandNull};
	sprintf((char *)bN58_AT_Cmd_List[Command25],"AT+HTTPACTION=99,%d",bLen);
	bN58_Data_Post[0]=bLen;
	for(i=0;i<bLen;i++)
		bN58_Data_Post[i+1]=bCode[i];
	N58_Load_Cmd(3,bN58_Cmd_List,bN58_Ack_List);
...
//HTTP Close 请求方法
void N58_Http_Close(void)
{
	u8 bN58_Cmd_List[1+1]={Command30,CommandNull};
	u8 bN58_Ack_List[1+1]={Command1,CommandNull};
	N58_Load_Cmd(2,bN58_Cmd_List,bN58_Ack_List);
}
	...
	if(!N58_Send_Struct.bInfBit.bSending_Flag)
	{
		if(N58_Send_Struct.bInfBit.bSend_Finish)
		{
			if(N58_Send_Struct.bInfBit.bFinish_state)
			{
				...
				if(N58_Send_Struct.bData_Mode)
					N58_Send_Struct.bResend_Count=1;
				else
					N58_Send_Struct.bResend_Count=3;
			}
			else
...
void N58_Usart_Send(u8 *bData,u16 bLen)
{
	u16 i;
	printf("发送的数据为：\r\n");
	for(i=0;i<bLen;i++)
	{
		USART_SendData(cN58_USART,bData[i]);
		while(USART_GetFlagStatus(cN58_USART,USART_FLAG_TC)==RESET); //循环发送,直到发送完毕 
		printf("%c",bData[i]);
	}
}
