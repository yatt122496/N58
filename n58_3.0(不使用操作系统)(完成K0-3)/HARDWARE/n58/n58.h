...
extern struct STRUCT_USART_Send	  //定义一个全局串口发送的处理结构体
{
	...
	u8 bNext_Task,bReSend_Count,bSend_Data_Flag;
	union
	{
		__IO u8 bInfAll;
    struct 
		{
		  __IO u8 bSend_Response   		 :1;                              // 0 发送应答标志

			__IO u8 bSend_Finish   			 :1;                              // 2 发送完成标志
	  }bInfBit;
  }; 
}N58_Send_Struct;
...
extern const u8 *bN58_AT_Ack_List[20];
...
u8 bN58_Tts_Set(u8 volume,u8 speed,u8 pitch,u8 format);
u8 *bN58_Tts_Play(u8 size, u8 *code);
