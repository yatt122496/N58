...
int main(void)
{
	...
	USART2_Init(115200);
 	LED_Init();			     //LED端口初始化
	TIM3_Int_Init(4999,7199);//10Khz的计数频率，计数到5000为500ms，每个计数用时 7200/72M=0.1ms
...
void Scan_Usart1(void)
{
	...
	if(USART_RX_STA&0x8000)
	{
		...
		{
			bN58_Tts_Set(3,3,3,0);
		}
		else if(strcmp((const char *)USART_RX_BUF,"K5")==0)
		{
			PC_USART("语音播报内容如下：\r\n%s\r\n",bVoice_chinese[i]);
			bN58_Tts_Play( strlen((const char *)bVoice_code_gbk[i] ) / 2 , (u8 *)bVoice_code_gbk[i] );
			i++;
...
void Scan_Usart2(void)
{
	u8 i,bResponse_Success=0,*bData=0;
	u32 dwNum_csq=0;
	if(N58_Fram_Record_Struct.wInfBit.wFramFinishFlag)
	{
		if(N58_Send_Struct.bInfBit.bSend_Response)
		{
			N58_Send_Struct.bInfBit.bSend_Response--;
			//在此添加需要特殊处理的应答
			if(strcmp((const char *)bN58_AT_Cmd_List[N58_Send_Struct.bTask_Cmd_List[N58_Send_Struct.bNext_Task]],"AT+CSQ")==0)
			{
			...
			{
				bData=bN58_Check_Data((u8 *)bN58_AT_Ack_List[N58_Send_Struct.bTask_Ack_List[N58_Send_Struct.bNext_Task]]);
			}
			if(bData||bResponse_Success)
			{
				N58_Send_Struct.bInfBit.bSend_Finish =1;
				N58_Send_Struct.bNext_Task++;			
				if(N58_Send_Struct.bTask_Cmd_List[N58_Send_Struct.bNext_Task] != 0)
				{
				...
				else
				{
					...
					for(i=0;i<N58_Send_Struct.bNext_Task;i++)
					{
			...
			{
				if(N58_Send_Struct.bReSend_Count--)
				{
					delay_ms(1000);
					N58_Send_AT_Cmd((u8 *)bN58_AT_Cmd_List[N58_Send_Struct.bTask_Cmd_List[N58_Send_Struct.bNext_Task]],
						(u8 *)bN58_AT_Ack_List[N58_Send_Struct.bTask_Ack_List[N58_Send_Struct.bNext_Task]]);
		...
		{
			N58_Fram_Record_Struct.bData_RX_BUF[N58_Fram_Record_Struct.wInfBit.wFramLength ] = '\0';
			if(N58_Send_Struct.bSend_Data_Flag)
			{
				PC_USART("DATA IS :\r\n%s\r\n", N58_Fram_Record_Struct .bData_RX_BUF);
				N58_Send_Struct.bSend_Data_Flag=0;
			}
			else
			{
				PC_USART("RECIEVE IS :\r\n%s\r\n", N58_Fram_Record_Struct .bData_RX_BUF);
