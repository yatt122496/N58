...
//	sprintf((char *)bCmd,"AT+TTSCFG=%d,%d,%d",bVolume,bSpeed,bPitch);			//语音播报配置
//	bCfg_flag=!N58_Send_AT_Cmd(bCmd,"OK",NULL,100,3);						//进行语音播报配置，若返回"OK"，则进行下一步，反之继续拨号，1s/次，最多3次，3次还不行则重启
//	
//	sprintf((char *)bCmd,"AT+TTSFMT=%d",bFormat);			//播报内容编码格式
...
//	sprintf((char *)bCmd,"AT+TTSPLAY=%d",bSize);			//语音播报配置
